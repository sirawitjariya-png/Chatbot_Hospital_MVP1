...
def grade_chunks(state):
    """CRAG: one LLM call decides whether retrieved chunks are relevant enough to answer.

    Replaces the old answer-then-evaluate-per-stage chain. If chunks are good,
    we generate ONE answer. If not, we fall through to web_search.
    """
    chunks = state.get("retrieved", [])
    if not chunks:
        return {
            "chunks_relevant": False,
            "trace": [{"node": "grade_chunks", "relevant": False, "reason": "no chunks"}],
        }
    ...
    return {
        ...
        "trace": [{"node": "grade_chunks", "relevant": relevant, "n_chunks": len(chunks)}],
    }
